do_wealth = function(tax_units, ...) { 
  ...
  deltas = read_csv(file.path('/gpfs/gibbs/project/sarin/shared/model_data/Wealth-Tax-Simulator/v1/2025011312', scenario_info$ID, 'detail', paste0(current_year, '.csv'))) %>%
    mutate(
    ...
    mutate(
      x = .13,
      delta_net_worth = min(0, ((x * (conventional - static) + (static - baseline)) / baseline) + .15)
    ) 
