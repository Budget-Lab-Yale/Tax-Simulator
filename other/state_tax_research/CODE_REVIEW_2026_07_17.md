...
| 7 | Generalize dense-table CSV; retire long YAML vectors | Suggestion | PARTIAL 2026-07-18: credit_tables schema generalized to (credit_id, state, year, filing_status, key_concept, income_lower, income_upper, value); CA's CSV migrated in place (20,738 rows, values untouched); VA CLI poverty table migrated as the pilot (integer-keyed, bit-identical). REMAINING QUEUE: CT Table E, KY family-credit table, CT retirement factors — each is income-banded, and migrating means adopting the CSV's INCLUSIVE integer-band convention where the YAML uses (lb, ub] on continuous income: behavior changes on fractional incomes between bands (arguably more form-faithful, but not bit-identical — decide deliberately, not as a refactor). Lookup rejects filing_status-keyed rows until that migration implements them. |
