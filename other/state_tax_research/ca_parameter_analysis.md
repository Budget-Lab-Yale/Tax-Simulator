...
The reusable, versioned independent-earned-credit table is now implemented. `config/scenarios/tax_law_state/baseline/ca/credit_tables.csv` stores all 20,738 nonzero FTB 3514 rows for 2017-25, including the 2017 footnote tails; `other/state_tax_research/refresh_ca_3514_tables.ps1` refreshes it from the official annual HTML. The generic calculator performs the earned-income lookup, the federal-AGI lookup above the annual child-count safe harbor, and selects the lower result. It uses the most recent published table for future years, so the 2025 table is an explicit carry-forward proxy after 2025. `n_dep_eitc`, `ei1 + ei2`, and federal AGI remain clearly documented proxies for California qualifying children, California earned income, and state eligibility.
