...
- **The CPS ASEC family already exists.** §4.1 says to "check `{production}/raw_data`
  ...
  `state_data_imputation_plan.md`'s new note prescribes.
...
- [x] **A2. Research pass A — ASEC tax-unit and income construction — DONE
      2026-08-19.** Deliverable: **`nonfiler_residual/10_asec_tax_unit_design.md`**,
      with every number reproducible from
      **`09_asec_tax_unit_diagnostics.R`** (tables `asec_A1..A6`).
...
- [x] **A4. Mok (2017) Table 14 — DONE 2026-08-19.** `resources/mok_coefs.csv`,
      **14 groups x 17 terms = 238 rows** with SEs, significance stars, group Ns
      ...
      instructed. Both documented Panel E traps confirmed (columns run
      **65+ first**, Ns 909 vs 62,438; `.` for self-employment in the 65+
      column) — plus **a third the note did not have: Panel E has no
      "Retirement income" row at all**, so its equations carry 15 covariates
      where every other panel carries 16. Placeholder cells are stored blank
      with a `note`, never as zero.
- [x] **A5. Cilke (1998) Table 3 — DONE 2026-08-19** (not skipped).
      `resources/cilke_coefs.csv`, **9 groups x 24 terms = 216 rows**, plus the
      WP-78 PDF committed as `resources/cilke1998_ota_wp78.pdf` (treasury.gov
      serves it to automated retrieval, unlike cbo.gov). **14 cells are published
      as `0.0000/0.0000` = not estimated** for that group; stored blank, and the
      count is asserted so a later edit cannot turn one into a real zero. Each
      group's equation spans two PDF pages, so the CSV carries `pdf_page` per row.
- [x] **A4/A5 verification.** `11_verify_coef_transcriptions.py` recomputes each
      PDF page's multiset of numeric tokens and checks every value the CSVs claim
      from that page — **454 values across 7 pages, all present**. It catches
      typos and dropped digits, not a same-page cell swap; that is what reading
      the images is for. **⚠ The two tables have OPPOSITE dependent variables:
      Mok predicts P(files), Cilke predicts P(does not file).** Mixing the
      conventions inverts a model while producing plausible probabilities.
...
- [ ] **F5. Validation additions** — the population-identity check (fitted filer
      ...
      `validate_state_weights.R`; re-run the EITC take-up correlation (the −0.61
...
- [ ] **F6. Re-fit** with config-7 hyperparameters (β=1e-4) unless the sweep says
      ...
      expansion ranked #1 in `state_weights_phase1_summary.md` §7, not by this.
...
- [ ] **H3. Send the cross-model issue docs upstream** (T1–T9 / P1–P5,
      ...
      but the longest-standing open item in `STATUS.md`.
