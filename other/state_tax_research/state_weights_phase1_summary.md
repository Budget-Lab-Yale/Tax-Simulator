...
`other/state_tax_research/sweep_state_weights.R` (hyperparameter sweep),
`other/state_tax_research/validate_state_weights.R` (validation battery),
`other/state_tax_research/build_state_weights.R` (end-to-end driver).
...
invariant leak documented in `state_weights_fit_issues.md`). `kg_amt` is
...
2. **Dual-space maxent (candidate production engine, from
   `state_weights_alternatives.md`).** Generalized raking on the ~12k
...
Rscript other/state_tax_research/sweep_state_weights.R --prior     # once
Rscript other/state_tax_research/sweep_state_weights.R <1..13>     # array
Rscript other/state_tax_research/sweep_state_weights.R --collect
Rscript other/state_tax_research/validate_state_weights.R --misses
Rscript other/state_tax_research/validate_state_weights.R --untargeted
Rscript other/state_tax_research/validate_state_weights.R --pilot
```
