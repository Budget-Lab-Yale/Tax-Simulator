# An integrated model for scoring taxes at the top

*The Budget Lab at Yale*

## 1. The idea

This memo describes the modeling enhancements we've made recently in service of producing more-accurate and internally consistent scores of packages aimed at taxing the rich. The basic idea is this: 
1. Top-tax packages often combine ordinary-income, capital-gains, business, estate, and/or wealth taxes in one proposal. _Example: a proposal to increase the corporate rate, increase capital gains rates, and tax unrealized gains at death._
2. These tax bases overlap both mechanically (e.g., a dollar of taxes paid by a corporation reduces the shareholder tax base) and behaviorally (e.g., reducing a tax rate differential across two substitutable tax bases should reduce avoidance opportunities). _Example: the corporate tax rate hike reduces after-tax profits which in turn reduces the tax base for the capital gains tax, while at the same time the tax on gains at death reduces the incentive to defer realizations, which raises the capital gains tax base._ 
3. Some of these interactions have no empirical evidence to rely on, since they arise from combinations of policies that have never been enacted. _Example: evidence on the realization elasticity is estimated solely under a policy regime of step-up; we have no directly observable estimate of how the elasticity changes if gains are taxed at death._ 
4. Therefore, to the extent possible, we should build one integrated microsimulation model with a ground-up, internally consistent representation of the relationship between business incomes, household incomes, household wealth, inheritance flows, etc., parameterized by behavioral rules grounded in economic theory and calibrated to current-law empirical elasticity moments. 

This approach represents an improvement over what we've done in the past, which is to build a set of standalone models and either ignore these interactions entirely or crudely assume relationships that might not hold under different policy regimes.

In what follows we describe how we model these mechanical and behavioral interactions. 
...
With this data in hand, we build several new modules in this simulator. These modules are functions which map policy changes to tax base changes (behavioral responses), or tax base changes to other tax base changes (mechanical interactions). The modules apply in a fixed sequence on shared data; we do not iterate them to a joint fixed point. Figure 1 maps the interactions these modules produce: each row is a tax increase, each column a base that moves in response. Appendix A gives the derivation behind each module.

**Figure 1 · How the tax bases interact**

| **An increase in…** | **Ordinary income** | **Capital gains** | **Corporate** | **Estate** | **Gains at death** | **Wealth** |
|---|---|---|---|---|---|---|
| **Ordinary rates** | ▼ *Underreporting rises; giving responds* | ▲ Compensation converts into deferred equity gains | ▲ Income shifts into C-corporation form | ▼ Evaders' assets leave the reported estate | ▲ Converted gains add to the stock later held at death | ▼ Evaders' assets leave the reported wealth base |
| **Capital gains rates** | ▲ Conversion unwinds; pay returns as salary | ▼ *Realizations deferred; retiming around effective dates* | ▼ All-in corporate rate rises; income shifts to pass-through | — | ▲ Deferred gains accumulate and arrive at death ° | — |
| **The corporate rate** | ▼ Rates of return competed down; interest, rents, pass-through income fall ° | ▼ Equity marked down; later realizations shrink ° | ▼ *Income shifts toward pass-through form* | ▼ Marked-down portfolios shrink estates ° | ▼ Unrealized gains absorb the markdown ° | ▼ Marked-down portfolios shrink the base ° |
| **The estate tax** | — | ▲ Holding until death shelters less; realizations rise | — | ▼ *Reported estates shrink* | ▼ Gains realized earlier leave less held at death | — |
| **The tax on gains at death** | ▲ Deferral worth less; conversion into gains unwinds | ▲ Deferral no longer escapes tax; realizations rise during life | ▼ Retain-to-death shelter loses value; income shifts to pass-through | ▼ Gains tax due at death is deductible from the estate ° | ▼ *Realizations pulled forward shrink the pool taxed at death* | — |
| **The wealth tax** | ▼ Concealed assets' income leaves the return | ▲ Carrying a deferred gain costs wealth tax; realizations rise | — | ▼ Concealed assets leave the reported estate | ▼ Earlier realizations leave fewer gains at death | ▼ *Valuation discounts and concealment shrink reported wealth* |

*And any of them:* every during-life tax increase is partly financed out of saving; the forgone wealth compounds, shrinking capital income, wealth, and estate bases in later years. °

▲ = the base grows; ▼ = the base shrinks. Italic (shaded in the print version) = own-base response. ° = mechanical interaction; others are behavioral responses. — = no first-order channel in the model.
...
Owners of businesses and certain workers can structure labor compensation as deferred equity gains rather than salary. The amount converted responds to changes in the gap between the taxpayer's ordinary rate and the present value of tax on a dollar entering the deferred-gain state. That present value comes from the realization piece above, so any policy that makes deferral less attractive makes this margin of conversion less attractive as well.

#### _Behavioral response:_ Moving between corporate and pass-through form
...
#### _Behavioral response:_ Underreporting of income

When marginal tax rates rise, some income leaves the tax return through evasion. Following
DeBacker, Heim, and Yuskavage (working paper presented at the National Tax Association meetings), reported income responds to the net-of-tax rate
where the IRS lacks third-party information reporting, i.e., in self-employment, partnership and S-corporation income, etc. The model adjusts these income forms when rates change and removes a matching share of closely held assets from the reported wealth and estate bases (assuming that income hidden from the income tax is not then presented to the wealth tax or the estate tax).
...
Evidence from other countries shows that reported wealth falls as the marginal tax rate on wealth rises. We assume this response is smaller for marketable assets than for closely held or other privately traded assets. We also assume that this response comes from two different margins. One is valuation discounts, which represent ways of lowering the assessed value of an asset that remains visible to the IRS. The other is concealment, for example the use of trust vehicles or offshore arrangements, which move the asset out of sight entirely, taking its
income out of this record's income tax base and its value out of the potential estate. Marketable securities have market prices that cannot be argued down, so their response is all concealment; the size of the response from closely held assets is assumed to be split evenly between the two components.

#### _Behavioral response:_ Estate tax avoidance and evasion

The estate tax is computed record by record from the same balance sheets used everywhere else, with valuation discounts, deductions, and spousal portability modeled explicitly and reported estates benchmarked to SOI filings (so avoidance already embedded in current-law estates is part of the baseline). When a reform raises the estate tax, reported estates shrink further. This is a pure reporting response (i.e., what heirs actually receive does not change), and the tax is passed to heirs in the microdata by matching large estates to large inheritances.

#### _Mechanical interaction:_ Tax-driven dissaving

Taxes are paid out of either forgone consumption or forgone
saving (assuming, as we do in conventional scores, that real income is fixed). If some fraction of new tax at time t is paid out of saving -- that is, equivalently, if MPCs are less than 100% -- then at time t+1, all else equal, there should be less private wealth (and lower estate value for those who die), less capital income flowing from that wealth, and thus less in income tax. Until this point, we implicitly assumed that all new taxes were paid out of consumption, since capital income flows were fixed across all scenarios.

We now have the machinery to vary this parameter (the fraction of new tax financed by lower saving) so that changes in taxes affect the path of wealth going forward. We assume this parameter varies with wealth: near the bottom of the
distribution, higher taxes mostly displace consumption (i.e., high MPCs), while at the top it
is financed almost entirely out of saving (i.e., low MPCs). The financed share then compounds: wealth
that is not accumulated stops earning returns, so the gap between baseline and
policy wealth widens over time. The consequences land in other bases years
later: less capital income to tax during life, a smaller wealth-tax base each
year, and smaller estates at death. A large during-life tax therefore
partially cannibalizes wealth-based revenue later in the budget window, and
...
## 3. How each piece is parameterized

Table 1 lists the parameters, grouped by module. It also has an "epistemic status" column with a rough, gut-level assessment of how confident we are in each parameter value. 

Most are taken directly from the empirical
literature but two are set differently. The first is η, the strength of the realization response in the realization model, calibrated so that the full model matches our preferred permanent realization elasticity of −0.6. The second is σ, the strength of ordinary-vs-gains income conversion,
set residually in a calibration exercise that targets a total elasticity of taxable income (ETI) of 0.25: we
run the model with all other behavioral margins turned on and find the value
of σ that delivers the target.
...
| Long-run response (η) — *realizations scale by the net-of-tax share of the deferral wedge raised to this power* | 1.63 | Calibrated on the full model to our preferred realization elasticity of −0.6 |
| Share of realizations that can retime ±1 year — *the fraction of each year's realizations free to shift one year toward the lower-rate year* | 0.25 | Calibrated to short-run elasticity of −1.2 |
| Valuation/compliance discount under deemed realization at death — *gains deemed realized at death are scaled down by this fraction before tax* | 25% | Calibrated to JCT score |
| **Income reporting and form** | | |
| Income conversion (σ) — *percent of the eligible compensation pool converted per point of change in the ordinary-versus-deferred-gain wedge* | 0.20 | Residual: set so the total top ETI is 0.25 (Saez, Slemrod, and Giertz 2012) |
| Entity-shifting semi-elasticity — *percent of pass-through business income changing form per point of change in the all-in rate differential* | 0.63 | Prisinzano and Pearce (2018) |
| Underreporting elasticities: self-employment / partnership & S-corp / rent — *percent change in reported income per one percent change in the net-of-tax rate* | 0.046 / 0.052 / 0.040 | DeBacker, Heim, and Yuskavage; zero under third-party reporting |
| Charitable giving price elasticity (cash) — *percent change in cash giving per one percent change in the price of giving* | −0.5 | Literature central value |
| **Wealth and estates** | | |
| Reported-wealth semi-elasticity: marketable / closely held — *reported wealth scales by exp(elasticity × marginal wealth-tax rate), by asset class* | −7 / −17 | Budget Lab review of international evidence |
| Concealment share of the avoidance response: marketable / closely held — *the concealed share exits the income and estate bases too; the valuation share exits reported wealth only* | 100% / 50% | Marketable assets have observable prices; private ones do not |
| Reported-estate net-of-tax elasticity — *the reported estate retains the net-of-tax ratio raised to this power* | 0.16 | Kopczuk and Slemrod (2001); range 0.10–0.22 |
| **Corporate revenue** | | |
| Corporate taxable-income elasticity (statutory, avoidance-only) — *the corporate base scales by the net-of-tax statutory ratio raised to this power* | 0.367 | Coles, Patel, Seegert, and Smith (2022) |
| **Mechanical interactions** | | |
| Share of new tax paid out of saving, by wealth rank — *the share of each new tax dollar paid out of saving; the forgone wealth compounds thereafter* | 0.04 → 0.80 | Consumption responses by wealth (Straub 2019 and related) |
| Corporate incidence constants (e.g., normal-return share 0.375) — *exposure and repricing constants in the incidence formulas of Appendix A.8* | various | Provisional placeholders pending direct measurement |

---

## Appendix A. Derivations for Section 2

The sections below follow the order of the modules in Section 2 (charitable giving, a one-parameter response, is documented in Appendix B). Each is self-contained: symbols are defined where they first appear and are local to their section.
...
Because unrealized gains are heavily concentrated, every cohort-level input — the marginal tax rate on gains, mortality, estate-tax exposure, the wealth-tax carrying cost defined below — is weighted by each household's share of the cohort's gains, so the cell parameters reflect the wealthy households who actually hold the gains. Mortality in particular is the probability that the marginal dollar's holder dies, well below the cohort's average. A record whose measured marginal rate on gains falls outside zero to one is dropped from its cell's average rate rather than carried into it, since a rate read off a bracket the record does not sit in is not a rate. Where the dropped records are more than one percent of a cell's weighted realizations, the run says so. The cohort-level response is then distributed back to tax units in proportion to a blend of their realized gains and their unrealized holdings.
...
where $r_B$ is the cohort's baseline realization rate and $\eta$ governs response strength. The cost is zero at the margin under baseline policy, $C'(r_B) = 0$, so baseline behavior is reproduced exactly. The wedge enters in log net-of-tax units: setting the marginal cost equal to $\ln(1 - MC_t) - \ln(1 - MC_{B,t})$ gives a closed form for the scenario realization rate,

$$
r \;=\; r_B \left(\frac{1 - MC_t}{1 - MC_{B,t}}\right)^{\!\eta}, \qquad \text{capped at } 1,
$$

where $MC_{B,t}$ is the marginal cost under baseline law. The response is a constant elasticity in the net-of-tax share of the full deferral wedge rather than in the current-year rate alone, the same functional form the empirical realization literature estimates, and it is undefined as the wedge approaches one, where the model stops rather than extrapolate.
...
We calibrate $\eta$ on the full model rather than on the cohort problem in isolation, so that the target elasticity is hit after mortality, death treatment, discounting, and the allocation to tax units are all accounted for. We run the simulator under a permanent capital-gains rate increase, measure the proportional realization response at a thirty-year horizon per unit change in the rate, and repeat across a small grid of $\eta$ values. The target is our preferred permanent realization elasticity of $-0.6$, the same local moment under either functional form; under the net-of-tax form the measured response is not linear in $\eta$, so the pin interpolates the grid piecewise rather than inverting a line through the origin. This gives $\eta = 1.6342$, which we quote as $1.63$; at the shipped value the model's implied long-run semi-elasticity at the current top rate is $-2.52$. The timeable fraction, $0.254$, is calibrated to a short-run realization elasticity of $-1.2$ around an announced rate change, measured in the year before a pre-announced increase takes effect, where law is unchanged and any movement is retiming alone. The deemed-realization discount of 25 percent is set so that the model's ten-year revenue from taxing gains at death matches the JCT score of that policy.
...
The module is organized around a single shock. The input is the gross change in corporate receipts, before any individual-tax offset. For a statutory-rate change that input is computed on the revenue side of the model itself: the CBO corporate receipts baseline implies the taxed base at the current rate, and the base erodes with an avoidance-only elasticity of corporate taxable income with respect to the statutory net-of-tax rate of 0.367 (Coles, Patel, Seegert, and Smith 2022) — deliberately not their larger effective-rate or total figures, since the policy lever is the statutory rate and the entity-form margin is modeled separately in Section A.3. Other corporate provisions arrive as off-model estimates. Holding aggregate pre-tax income fixed, a corporate tax increase reduces after-tax profits dollar for dollar, so $w_t$ — read directly from the scored receipts change — corresponds conceptually to $\Delta\tau_t\,\Pi_t$ for pre-tax profit $\Pi_t$ and rate change $\Delta\tau_t$. Everything below traces this one reduction through the model: as current income flows, as asset values, and finally as its consequence for individual receipts.
...
This section expands the one-line "Basis" column of Table 1. For each parameter we give the underlying citation, describe how the published estimate maps into the model's units, and, where the value is calibrated rather than taken from the literature, describe the calibration and date it. Precise pinned values are given here; Table 1 rounds them.

**Capital gains realization.**

**Long-run response (η).** The pinned value is 1.6342; Table 1 rounds to 1.63. η is the elasticity in the realization rule — each cell's discretionary realization rate scales by the net-of-tax share of the deferral wedge raised to the power η — and because the whole gain pool responds, it carries the model's aggregate long-run response; the implied long-run semi-elasticity at the shipped value and the current top rate is −2.52. It is calibrated so that the full model reproduces our preferred permanent realization elasticity of −0.6, which sits inside the empirical range: Dowd, McClelland, and Muthitacharoen (2015) estimate a persistent elasticity of −0.72 (with a within-year elasticity of −1.2) on a large panel of returns, Agersnap and Zidar (2021) find ten-year revenue elasticities of −0.3 to −0.5 with revenue-maximizing rates of 38 to 47 percent, and the JCT and CBO scoring conventions sit in the same neighborhood. For the calibration (July 2026), we ran a permanent increase in the top statutory rate on gains through the full model at several trial values of η and measured the realized long-run response at simulation year 30. Under this functional form the measured response is not linear in η, so the pin interpolates the measured grid piecewise onto the target moment (−0.6, converted at the 23.8 percent top federal rate on long-term gains) rather than inverting a line through the origin. The grid and fit are kept in the calibration records so that future re-derivations are arithmetic. The value was last re-derived in July 2026, after the cell average began dropping records whose measured marginal rate on gains falls outside zero to one; that rule lowers the tax price in the cells where those records landed, and the pin moved with it. An earlier specification expressed the same rule as a constant semi-elasticity in the wedge itself, calibrated to the same moment at η = 2.48; the two forms agree near current rates and part ways at high ones, where the net-of-tax form implies stronger deferral and a lower revenue-maximizing rate under step-up.

**Share of realizations that can retime ±1 year.** Pinned at 0.2542; Table 1 rounds to 0.25. This is the fraction of baseline realizations free to shift across adjacent years toward the lower-rate year, and it carries the model's short-run announcement response. It is calibrated (July 2026) to our preferred short-run elasticity of −1.2, which is the within-year estimate in Dowd, McClelland, and Muthitacharoen (2015). Because the retiming margin nets to zero under a uniform permanent rate change, η is pinned first and independently; given η, the timing share is then measured in the year before a pre-announced rate increase takes effect — law is unchanged that year, so any movement is retiming alone — and a monotone grid over the share is interpolated onto the short-run moment (target semi-elasticity 1.2/0.238 = 5.04).
...
**Income conversion (σ).** σ = 0.20 governs how much top-bracket labor-type compensation is restructured into unrealized equity gains when the wedge between the ordinary rate and the equity path's expected present-value tax rate widens: 0.20 percent of the eligible compensation pool converts per percentage point of wedge. It is the model's one residual parameter. With every other reporting margin active (realization, entity shifting, underreporting, charity), we run a five-point top-rate increase through the full model, measure the total elasticity of top taxable ordinary income, and set σ so that this lands on 0.25 — the central reading of Saez, Slemrod, and Giertz (2012), whose survey puts the best available estimates between 0.12 and 0.40. The value was re-derived in July 2026 under the production charitable elasticity of −0.5: with conversion off, the other margins produce an ETI of 0.1820, and a three-point grid over σ (0, 0.16, 0.30, measuring 0.1820, 0.2363, 0.2840) interpolates onto the target at σ = 0.2002, shipped as 0.20. Because σ is residual, it is re-derived whenever any other margin in the stack changes; a standing check in the calibration records enforces this.
...
gross of estate tax, so only the liability changes across scenarios.
...
to current ones. The valuation bridge, deductions,
...
Coles, Jeffrey L., Elena Patel, Nathan Seegert, and Matthew Smith. 2022. "How Do Firms Respond to Corporate Taxes?" *Journal of Accounting Research* 60 (3): 965–1006.
...
DeBacker, Jason, Bradley Heim, and Alexander Yuskavage. 2025. "Marginal Tax Rates and Income Tax Noncompliance." Presented at the National Tax Association annual meetings, November 2025.
...
Prisinzano, Richard, and James Pearce. 2018. "Tax Based Switching of Business Income." Penn Wharton Budget Model Working Paper W2018-2.
