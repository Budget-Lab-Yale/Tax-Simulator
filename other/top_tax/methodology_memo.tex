...
Long-run response ($\eta$)\pform{realizations scale by the net-of-tax share of the deferral wedge raised to this power} & 1.63 & Calibrated on the full model to our preferred realization elasticity of $-0.6$ & High \\
\addlinespace[0.55em]
...
the marginal dollar's holder dies, well below the cohort's average. A record
whose measured marginal rate on gains falls outside zero to one is dropped
from its cell's average rate rather than carried into it, since a rate read
off a bracket the record does not sit in is not a rate. Where the dropped
records are more than one percent of a cell's weighted realizations, the run
says so. The cohort-level response is then distributed back to tax units in
proportion to a blend of their realized gains and their unrealized holdings.
...
rather than inverting a line through the origin. This gives $\eta = 1.6342$,
which we quote as $1.63$; at the shipped value the model's implied long-run
semi-elasticity at the current top rate is $-2.52$. The timeable fraction,
$0.254$, is calibrated to a short-run realization elasticity of $-1.2$ around
...
\textbf{Long-run response ($\eta$).} The pinned value is 1.6342; Table 1
rounds to 1.63. $\eta$ is the elasticity in the realization rule---each
...
$-2.52$. It is calibrated so that the full model reproduces
...
the calibration records so that future re-derivations are arithmetic. The
value was last re-derived in July 2026, after the cell average began dropping
records whose measured marginal rate on gains falls outside zero to one; that
rule lowers the tax price in the cells where those records landed, and the pin
moved with it. An
