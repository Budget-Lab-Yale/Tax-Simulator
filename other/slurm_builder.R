...
build_slurm = function(runscript_name, user_id, stamp = NULL, scenario_id = 'baseline', local = 1,
                       vintage = NULL, pct_sample = 1, stacked = 0, baseline_vintage = NULL,
                       delete_detail = 1, multicore = 0) {
  library(processx)
  
  run_args = paste(runscript_name, 
    paste0('/gpfs/gibbs/project/sarin/', user_id, '/Repositories/Tax-Simulator/src/main_parallel.R'), 
    scenario_id, local, ifelse(is.null(stamp), vintage, stamp), pct_sample, stacked, 
    ifelse(is.null(stamp), baseline_vintage, stamp), delete_detail, multicore,
    sep = " "
  )
  
  run("module", "load miniconda")
  run("conda", "activate ybl-rbash")
  start = Sys.time()
  run("Rscript", run_args)
  end = Sys.time()
  ...
  for(i in 1:splits){
    ...
    cat(paste0('#!/bin/bash',
               '\n#SBATCH --array=',(10000*(i-1))+1,'-', ifelse(i==splits, nrow(scripts)/2, 10000*i),
               '\n#SBATCH --job-name batch-',stamp,
               '\n#SBATCH --output=/vast/palmer/scratch/sarin/', user_id, '/Tax-Simulator/config/batch-submissions/',stamp,'/output/slurm-%A_%a.out',
               '\n#SBATCH --error=/vast/palmer/scratch/sarin/',  user_id, '/Tax-Simulator/config/batch-submissions/',stamp,'/output/slurm-%A_%a.err',
               '\n#SBATCH --mem-per-cpu 10g \n#SBATCH --time=',splits,':30:00', 
               '\n#SBATCH --partition scavenge \n#SBATCH --requeue',
               '\nmodule load miniconda \nconda activate ybl-rbash',
               '\nfile=/gpfs/gibbs/project/sarin/', user_id, '/Repositories/Tax-Simulator/config/batch-submissions/',stamp,'/batch_array.txt',
