...
# calibrate_eta_v2.R
#
# Cell-MTR-aware eta calibration. Designed to be run AFTER a full-sample
# baseline simulator pass that has registered mtr_vars = "kg_lt" (so baseline
# static detail carries mtr_kg_lt per record).
#
# Methodology — discussed with user, May 2026:
...
#
# Output: prints recommended eta. User pastes into the eta default in
# kg_dyn_run_bathtub_pass() (src/sim/kg_dynamics.R).
#
# CLI:
#   Rscript other/kg_model_tests/calibrate_eta_v2.R <baseline_root>
#
...
if (length(args) < 1) {
  stop('Usage: Rscript other/kg_model_tests/calibrate_eta_v2.R <baseline_root>')
}
