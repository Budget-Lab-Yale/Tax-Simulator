...
detail_policy   = read_detail('surtax') %>% mutate(scenario = 'Policy')
...
rev_stacked = rev %>%
  ...
  select(year, `Alt Max Tax`, Surtax, Total)
...
provisions = c('Alt Max Tax', 'Surtax', 'Total')
...
make_window = function(df, start, end, label) {
  ...
  tibble(
    year           = label,
    `Alt Max Tax`  = sum(sub$`Alt Max Tax`),
    `Surtax`       = sum(sub$Surtax),
    `Total`        = sum(sub$Total),
    gdp_fy         = gdp_total
  )
}
...
title = textGrob('Table 1. Stacked Revenue Estimates: Working Americans\' Tax Cut Act',
