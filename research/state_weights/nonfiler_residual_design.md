...
| Pub 5785 above-threshold composition | hand-transcribed CSV with page citations | **transcribed 2026-08-16** — `research/state_weights/nonfiler_residual/resources/pub5785_table1_potential_nonfilers.csv` (person level) and `pub5785_table3_notfiler_units.csv` (unit level). The single `pub5785_hazard.csv` this row used to name was a planning name and never existed |
| **Mok (2017) Table 14 probit coefficients** — the primary below-threshold model (§3.2.2) | **transcribed 2026-08-19** from rendered page images: **14 groups x 17 terms = 238 rows** with SEs, stars, group Ns and weighted filing rates. Panel E's reversed column order and its `.` for self-employment both confirmed; a third irregularity found — **Panel E has no retirement-income row at all**, so it carries 15 covariates, not 16 | `research/state_weights/nonfiler_residual/resources/mok_coefs.csv`, verified page by page by `11_verify_coef_transcriptions.py` |
| Cilke (1998) probit coefficients — retained as the **comparison** fit only (§3.2.2) | **transcribed 2026-08-19** from rendered page images: **9 groups x 24 terms = 216 rows**, with the 14 cells published as `0.0000/0.0000` stored blank because they are *not estimated* for that group, not zero. **⚠ Cilke predicts P(non-filer); Mok predicts P(files)** — opposite conventions, never pooled without conversion | `research/state_weights/nonfiler_residual/resources/cilke_coefs.csv`, with WP-78 committed alongside as `cilke1998_ota_wp78.pdf` |
...
- **2026-08-19** — SSA inputs placed, verified and documented; five pre-flight
  ...
  **Consolidated**: amendments folded into the body, change log replaced by this
  ...
  `research/state_weights/plan.md`.
- **2026-08-19 (later)** — §5's Table 5 paragraph amended: the covered-worker
  age detail is published on the **total** covered universe, not on wage-and-salary
  (A1). Data-inventory rows for the three transcribed coefficient sets brought up
  to date (A4/A5): Mok and Cilke are now transcribed and verified, and the Pub 5785
  row no longer names `pub5785_hazard.csv`, a planning name for a file that was
  never created — the transcription landed as two files, by table.
