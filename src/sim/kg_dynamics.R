...
#   'levels' (DEFAULT) -- constant SEMI-elasticity: log(r_D) is linear in the
#       structural wedge MC, r_D = r_D_B * exp(-eta*(MC - MC_B)). The joint
#       consequence of a linear payoff + KL/entropy realization cost; eta IS
#       the long-run semi-elasticity. This is the shipped v3 calibration and
#       is BIT-IDENTICAL to the pre-toggle code path.
#   'logs' -- constant NET-OF-TAX elasticity (Agersnap-Zidar's preferred
#       spec): log(r_D) is linear in log(1 - MC),
...
#       independent calibration (eta_tilde AND timeable share); NA until pinned.
...
KG_DYN_RESPONSE_FORM    = local({
  v = Sys.getenv('KG_RESPONSE_FORM', 'levels')
