...
process_for_distribution = function(id, baseline_id, yr, corp_rate_delta, 
  ...
  microdata = baseline %>%
    ...
    mutate(
      ...
      corp_tax_labor   = corp_rate_delta * 1e9 * corp_rate_labor_share       * (labor / sum(labor * weight)),
      corp_tax_capital = corp_rate_delta * 1e9 * (1 - corp_rate_labor_share) * (capital / sum(capital * weight)),
      ...
      delta = (liab - liab_baseline) + other_taxes,
    ...
    for (group_var in c('income', 'age')) {
      ...
      for (include_other in c(F, T)) {
        ...
        results[[group_var]][[length(results[[group_var]]) + 1]] = dist_metrics %>%
          ...
          mutate(includes_other = include_other, .after = year)
        
        # Add to Excel workbook and format
        format_table(dist_metrics, wb, yr, paste0(group_var, '_group'), include_other)
